...
public class Aplicacion {
	...
	private void agregarElementoPedido() {
			...
			for (int i=0;i<restaurante.getMenuBase().size();i++) 
		{
			...
			int opcionProducto = Integer.parseInt(input("Ingrese el numero de el producto que desea"));
			System.out.println("Desea agregar un ingrediente?");
			System.out.println("No Agregar(0)");
			System.out.println("Agregar(1)");
			String opcionIngrediente=sc.nextLine();
			Producto product = new ProductoAjustado(null);
			...
			if(opcionIngrediente.equals("1")) {
				...
				while(empezarCiclo){
					...
					{
						      int precio =restaurante.getIngredientes().get(x).getCostoAdicional();
						      System.out.println(x+1+"."+restaurante.getIngredientes().get(x).getNombre()+" "+precio);
					}
				opcionProducto = Integer.parseInt(input("Ingrese el numero de el ingrediente que desea agregar"));
				productoFinal.añadirIngrediente(restaurante.getIngredientes().get(opcionProducto-1));
				String mas =input("Desea agregar algun ingrediente mas?\nNo Agregar(0)\nAgregar(1)");
				if(mas.equals("0")) {
					empezarCiclo = false;
					String remover=input("Desea remover algun ingrediente?\nNo (0)\nSi(1)");
					if(remover.equals("1")) {
						...
						while (cicloRemover) {
							...
							opcionProducto = Integer.parseInt(input("Ingrese el numero de el ingrediente que desea remover"));
							productoFinal.eliminarIngrediente(restaurante.getIngredientes().get(opcionProducto-1));
							mas =input("Desea eliminar algun ingrediente mas?\nNo (0)\nSi(1)");
							if(mas.equals("0")) {
								cicloRemover = false;
								String acabarPedido =input("Desea algun producto mas?\nNo (0)\nSi(1)");
				...
				if(remover.equals("0")) {
					...
					while (cicloRemover) {
						...
						opcionProducto = Integer.parseInt(input("Ingrese el numero de el ingrediente que desea remover"));
						productoFinal.eliminarIngrediente(restaurante.getIngredientes().get(opcionProducto-1));
						 String mas =input("Desea eliminar algun ingrediente mas?\nNo (0)\nSi(1)");
						if(mas.equals("0")) {
							cicloRemover = false;}}
